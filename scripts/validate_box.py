...
Validates the shipped examples and every contributor `box.yaml` file. Starter
...
8. Each competition and box identity maps to a unique storage namespace.
...
def is_reserved(path: Path) -> bool:
    try:
        ...
        return False
    return bool(relative_path.parts and relative_path.parts[0] in RESERVED)
    ...
    if roots:
        for root in roots:
            if root.is_file():
                ...
                candidates = sorted(root.rglob("box.yaml"))
            for path in candidates:
                resolved = path.resolve()
                if not is_reserved(resolved):
                    yield resolved
        return
    ...
    for host in doc["hosts"]:
        ...
        if role == "domain-controller":
            ...
            if source and image:
                ...
                errors.append(
                    f"host '{name}': only a VM can use build.image"
                )
